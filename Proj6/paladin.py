    ...
    def attack(self) -> int:
        ...
        if (attack_type == 1):
            if (self.d20.roll() >= 13):  # do we hit?
                ...
                print(f"Paladin hits with mace for {damage}")
            else:
                print(f"Paladin misses with mace")

        elif (attack_type == 2):
            if (self.d20.roll() >= 16):
                damage = self.d6.roll() + self.d6.roll() + self.d6.roll()  # 3d6
                print(f"Paladin hits with shield for {damage}")
            else:
                print(f"Paladin misses with shield")
        elif (attack_type == 3):
            self.attack_buff += 0.1
            print(f"The Paladin buffs their attack by 10% for a total modifier of {self.attack_buff}")
                

        else:
            damage = -1 * self.d6.roll()
            print(f"Paladin heals for {-1*damage}")
            

        # return the damage
        damage = math.ceil(damage*self.attack_buff)
        return damage
